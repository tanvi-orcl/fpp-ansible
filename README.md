...
- Runtime Variables
    ...
    - OPTIONAL: db_home_name `dbhome2_191300` - specify custom db_home_name if default name already exists 
    - OPTIONAL: osdbagrp_groups `OSDBA=dba,OSOPER=racoper,OSDG=dba,OSKM=dba,OSRAC=dba`
...
- Runtime Variables
    ...
    - image_name `DB191200`
    - db_home_name `dbhome1_191200` - name of existing database home to add as working copy
    - OPTIONAL: osdbagrp_groups `OSDBA=dba,OSOPER=racoper,OSDG=dba,OSKM=dba,OSRAC=dba`
...
- Runtime Variables
    ...
    - OPTIONAL: orphan_home `true` - set orphan_home to true if home does not exist in FPP (i.e. if exists in the vm cluster but is not a registered working copy)

**gi_create_image.yml**
- Registers a new grid image to FPP. Run once per image so be sure the hostgroup specified (exa_host) only contains one cluster. The host should have an active grid home that has already been patched to the correct version and is the image to be saved.
- Runtime Variables 
    - exa_host `exadata1n1`
    - container_url - repo to host zipped GI image
        - curl_username
        - curl_password
...
- Runtime Variables
    - exa_group `exadata1n1,exadata2n1`
    - container_url - repo to pull zipped GI image
        - curl_username
        - curl_password
        - OPTIONAL: curl_https_proxy
    - image_name `GI191300`
    - OPTIONAL: gi_home_suffix `grid191300_1` - path suffix to create custom gi home path instead of FPP generated path
    - OPTIONAL: wc_name `wc_GI191300_exadata1_2` - specify custom wc_name if default name already registered 
...
FPP Documentation: https://docs.oracle.com/en/database/oracle/oracle-database/19/fppad/preface.html#GUID-AD2CB91D-7FEB-485C-AC92-C4343731EA73
